...
class MnistData(DataProcess):
    ...
    def train_dict(self):
        train_data, train_label = self._raw_mnist_data.train.next_batch(100)
        return {self._abs_data: train_data,
                self._abs_label: train_label}

    def valid_dict(self):
        valid_data, valid_label = self._raw_mnist_data.validation.next_batch(100)
        return {self._abs_data: valid_data,
                self._abs_label: valid_label}

    def test_dict(self):
        test_data, test_label = self._raw_mnist_data.test.next_batch(100)
